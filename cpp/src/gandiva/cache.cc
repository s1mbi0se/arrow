...
void LogCacheSizeSafely(size_t capacity, size_t disk_capacity, size_t reserved) {
  ARROW_LOG(INFO) << "Creating gandiva cache with memory capacity of " << capacity << " bytes";
  ARROW_LOG(INFO) << "Creating gandiva cache with disk space of " << disk_capacity << " bytes";
  ARROW_LOG(INFO) << "Creating gandiva cache with reserved disk space of " << reserved << " bytes";
}
